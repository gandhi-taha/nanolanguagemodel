...
def truncate_prompt(prompt):
    """
    Cuts the prompt down to size if it's too long.
    """
...
def list_tokens(prompt):
    """
    Shows you the tokens for a given prompt.
    """
...
def generate_ts(engine, prompt, max_tokens=200):
    """
    Gets a response from a TextSynth server.
    
    Set DAISYNLM_TS_SERVER and DAISYNLM_TS_KEY env vars to use this.
    """
    apikey = os.environ.get("DAISYNLM_TS_KEY") or ""
    server = os.environ.get("DAISYNLM_TS_SERVER") or "https://api.textsynth.com"
...
def generate_oa(engine, prompt, max_tokens=200, temperature=0):
    ...
    Set DAISYNLM_OA_KEY env var to use this.
    """
    apikey = os.environ.get("DAISYNLM_OA_KEY")
...
def generate_ts(engine, prompt, max_tokens=200):
    ...
    DAISYNLM_TS_SERVER is the server such as http://localhost:8080
    DAISYNLM_TS_KEY is the API key
    """
    apikey = os.environ.get("DAISYNLM_TS_KEY") or ""
    server = os.environ.get("DAISYNLM_TS_SERVER") or "https://api.textsynth.com"
...
def generate_oa(engine, prompt, max_tokens=200, temperature=0):
    ...
    DAISYNLM_OA_KEY is the API key
    """
    apikey = os.environ.get("DAISYNLM_OA_KEY")
...
def generate(
    ...
    ['...Paris...']
    """
    if os.environ.get("DAISYNLM_TS_KEY") or os.environ.get(
        "DAISYNLM_TS_SERVER"
    ):
        return generate_ts("flan_t5_xxl_q4", instructions, max_tokens).strip()

    if os.environ.get("DAISYNLM_OA_KEY"):
