...
from daisynlm.preprocess import get_html_paragraphs
from daisynlm.inference import (
    generate,
    rank_instruct,
    parse_chat,
    list_tokens,
)
from daisynlm import embeddings
...
def get_web(url: str) -> str:
    ...
    res = requests.get(
        url, headers={"User-Agent": "Mozilla/5.0 (compatible; daisynlm)"}
    )
