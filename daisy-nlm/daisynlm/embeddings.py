...
def embed(docs):
    """
    Calculates embeddings for a list of documents.
    
    We process these in batches of 64 to keep things moving fast without
...
def search(query, docs, count=16):
    """
    Finds the top `count` documents that match your query.
    """
...
def get_token_ids(doc):
    """
    Gets the token IDs for a document using the generative model's tokenizer.
